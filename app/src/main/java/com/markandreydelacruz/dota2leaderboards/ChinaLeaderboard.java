...
public class ChinaLeaderboard extends Fragment {

    View rootView;
    private ListView listViewChinaLeaderboards;

    @Override
    public View onCreateView(LayoutInflater inflater, final ViewGroup container, Bundle savedInstanceState) {
        rootView = inflater.inflate(R.layout.activity_chinaleaderboard, container, false);

        if(MainActivity.isNetworkAvailable(getContext())) {
            getChinaLeaderboards(MainActivity.URL_CHINA_FEED);
        } else {
            Toast.makeText(getContext(), "No Internet Connection", Toast.LENGTH_LONG).show();
        }

        listViewChinaLeaderboards = (ListView) rootView.findViewById(R.id.listViewChinaLeaderboards);

        return rootView;
    }

    private void getChinaLeaderboards(String urlChinaFeed) {
        ...
        asyncHttpClient.get(getActivity(), urlChinaFeed, new AsyncHttpResponseHandler() {
            ...
            public void onSuccess(int statusCode, Header[] headers, final byte[] responseBody) {
                ...
                try {
                    ...
                    listViewChinaLeaderboards.setAdapter(adapter);
                    if (this.dialog.isShowing()) {
                        this.dialog.dismiss();
                    }
